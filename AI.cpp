...
namespace sOLO
{
	...
	void AI:: PlacePiece(int(*gridArray)[3][3], std::optional<sf::Sprite> gridPieces[3][3], int* gameState) {
		try
		{
	...
	void AI::CheckSection(int x1, int y1, int x2, int y2, int X, int Y, int pieceToCheck, int(*gridArray)[3][3], std::optional<sf::Sprite> gridPieces[3][3])
	{
	...
	void AI::CheckIfPieceIsEmpty(int X, int Y, int(*gridArray)[3][3], std::optional<sf::Sprite> gridPieces[3][3])
	{
		if (EMPTY_PIECE == (*gridArray)[X][Y])
		{
			(*gridArray)[X][Y] = AI_PIECE;
